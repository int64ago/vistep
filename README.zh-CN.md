...
**本次发布：** 合集扩展至 **64 篇**，正式站通过 `main` 的 [GitHub Actions](https://github.com/int64ago/vistep/actions/workflows/ci.yml?query=branch%3Amain) 发布。[合集扩展记录](docs/zh-CN/expansion-50.md)和[各篇审看记录](docs/zh-CN/README.md#检查记录)分别说明交付状态，以及尚未覆盖全部专题的完整观看、原生试听和真机检查。
...
使用 [Node.js 24](.nvmrc) 与 pnpm 11.25.0。配置的浏览器基线为 Chrome 108、Edge 108、Firefox 121、Safari 15.4；[能力检查](docs/zh-CN/architecture.md#浏览器支持)发现必要功能不可用时，会显示提示并停止加载实验。配音录音存放在 [Git LFS](https://git-lfs.com/)，克隆前先安装。
