...
import Scene from './DifferentialStudio';
import { differentialShot, differentialMotion } from '../../models/differential';
...
function shot(chapter: number, progress: number) {
  const s = differentialShot(chapter, progress);
  return { motion: s.motion, focus: s.focus, progress };
}
...
describe('Differential exploration framing', () => {
  it('keeps the actual assembly in frame after creation, resize and permitted orbit directions', () => {
    const v = create([1150 / 440, 278 / 260, 278 / 300][0]);
    try {
      // Equivalent to returning from 2D: new create followed by Studio.resize.
      Object.assign(v.ref.current, {
        motion: differentialMotion('turn', 2, 8, 0.8, -1),
        focus: 'spiders',
        progress: 0.5,
      });
      v.object.update();
      for (const aspect of [1150 / 440, 278 / 260, 278 / 300]) {
        v.resize(aspect);
  ...
  it('fits the last actual drag update before render, and releases its control listener', () => {
    ...
    try {
      ...
      v.object.update();
      v.resize([1150 / 440, 278 / 260, 278 / 300][0]);
    ...
    try {
      for (let chapter = 0; chapter < 8; chapter++)
        for (const progress of [0, 0.001, 0.2, 0.5, 0.8, 0.999, 1]) {
          ...
          v.controls.update();
          const subject =
            chapter === 2 && progress > 0 && progress < 1 ? v.root.children[1] : v.root;
  ...
  it('reconstructs directed camera state after exploring and seeking in a different order', () => {
    const v = create([1150 / 440, 278 / 260, 278 / 300][0], true);
    ...
    try {
      ...
      v.object.update();
      v.resize([1150 / 440, 278 / 260, 278 / 300][1]);
      expect(peak(v)).toBeLessThanOrEqual(0.881);
      v.resize([1150 / 440, 278 / 260, 278 / 300][0]);
      seek(7, 0.91);
      seek(0, 0);
