...
import { fitWirelessCamera, wirelessFramingPoints } from './wirelessChargingFraming';
...
export default function WirelessChargingStudio({
  ...
  return (
    <Studio
      ...
      create={(context) => createWirelessApparatus(latest, narrow, context)}
    />
  );
}

// Exported so projection checks can instantiate the exact rendered apparatus without WebGL.
export function createWirelessApparatus(
  latest: { current: WirelessShot },
  narrow: boolean,
  ...
  const put = (g: THREE.BufferGeometry, m: THREE.Material, p: WCPoint, group: THREE.Object3D) => {
  ...
  const wire = (
    ...
    for (const p of points) put(new THREE.SphereGeometry(r * W.scale, 10, 7), material, p, group);
  };
  ...
  for (const [receiver, group] of [
    ...
    if (receiver) {
      ...
      for (const p of wires.bridge) put(new THREE.SphereGeometry(0.045, 10, 8), mat, p, group);
  ...
  const fluxLines = Array.from({ length: 4 }, (_, i) => {
    ...
    geometry.setAttribute('position', new THREE.BufferAttribute(new Float32Array(129 * 3), 3));
  ...
  return {
    update() {
      ...
      for (const item of fluxLines) {
        item.line.visible = item.arrow.visible = sh.showFlux && Math.abs(s.instantFlux) > 1e-9;
        if (key !== geometryKey) {
      ...
      const nextFrameKey = `${key}/${sh.input.connected}`;
      if (nextFrameKey !== frameKey) {
        framePoints = wirelessFramingPoints(root);
        frameKey = nextFrameKey;
      }
      // Run after Studio's resize and before render, including paused deep links.
      // Refit the same geometry for keyboard/orbit angles as well as viewport changes.
      fitWirelessCamera(camera, controls.target, framePoints);
    },
  };
}
