...
import Scene from './BallBearingStudio';
...
function shot(chapter: number, progress: number) {
  const s = bearingShot(chapter, progress);
  return { state: bearingState(s.innerAngle, s.loadAngle, s.load), shot: s };
}
...
describe('BallBearing exploration framing', () => {
  it('keeps the actual assembly in frame after creation, resize and permitted orbit directions', () => {
    const v = create([643 / 500, 246 / 310, 254 / 318][0]);
    try {
      // Equivalent to returning from 2D: new create followed by Studio.resize.
      Object.assign(v.ref.current, {
        state: bearingState(-6 * Math.PI, Math.PI, 1),
        shot: { ...bearingShot(2, 0.5), innerAngle: -6 * Math.PI, cutaway: 1 },
      });
      v.object.update();
      for (const aspect of [643 / 500, 246 / 310, 254 / 318]) {
        v.resize(aspect);
  ...
  it('fits the last actual drag update before render, and releases its control listener', () => {
    ...
    try {
      ...
      v.object.update();
      v.resize([643 / 500, 246 / 310, 254 / 318][0]);
  ...
  it('reconstructs directed camera state after exploring and seeking in a different order', () => {
    const v = create([643 / 500, 246 / 310, 254 / 318][0], true);
    ...
    try {
      ...
      v.object.update();
      v.resize([643 / 500, 246 / 310, 254 / 318][1]);
      expect(peak(v)).toBeLessThanOrEqual(0.881);
      v.resize([643 / 500, 246 / 310, 254 / 318][0]);
      seek(7, 0.91);
      seek(0, 0);
