...
Use the existing narration production pipeline to generate static recordings, and commit its scripts, recordings and generated manifests together. Synthesis uses the network during production; ordinary builds and readers need no speech credentials or live AI service. Do not substitute browser text-to-speech for the authored recordings.
...
Keep cover geometry shared across its actual entry points while composing each format separately. Inspect the catalog, applicable homepage fallback and generated social/collection image; a correct scene model does not prove that its cover still contains the mechanism or preserves contact and occlusion.
...
When a request adds multiple scenes, proactively use subagents and available concurrency, preferably with an independent owner for each scene. Assign its objective, applicable skill, owned files and acceptance requirements. Research, model review and visual/interaction review may also run as bounded independent tasks. The main agent owns overall planning, shared-file coordination, integration and per-scene acceptance; a worker's completion claim does not replace actual review and verification.

Keep a bounded queue across creation, review and repair. After each frozen handoff, the integrator owns those files and assigns the worker another ready, independent task. Disclose author self-review; do not label it independent review. Keep shared translations, narration and manifests under one writer. The optional packet helper does not complete discovery registration, source catalogs, recorded speech or publication. A blocked browser check leaves an explicit evidence gap while independent model, source or document work continues. See the [parallel workflow](../../../docs/creating-a-scene.md#register-the-scene).

## Review and deliver

Run the checks appropriate to the change. Complete topics require `pnpm scene:check`, `pnpm verify` and `pnpm build:preview`. Independently inspect complete playback, key stills and contacts, both spoken tracks, chapter/seek boundaries, extreme inputs, desktop and narrow layouts, keyboard/touch operation, reduced motion and applicable resource failures. Measure the complete player, including its longest caption and transport, in both languages. Inspect actual rendered text size after SVG scaling, perspective near corners and moving assembly bounds; distinguish intentional close-ups from accidental cropping. Body text is at least 16 px; primary touch targets are at least 44 px. Test deep links and 404s. Check startup capability rejection and readable static content separately from runtime WebGL failures after the gate passes; preserve an appropriate 2D path for those runtime failures, without claiming that rejected browsers run the experiments.
...
For any agent-created or rewritten commit, including amend, squash and merge commits, follow [AGENTS.md](../../../AGENTS.md) and its accurate model/provider `Co-Authored-By` requirements. Preserve original authorship and correct existing trailers, then inspect the final message.
