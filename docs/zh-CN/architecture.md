...
```text
src/data/topics.ts ──→ Astro 静态路由 /zh/ 与 /en/ ──→ 目录、元数据、相关专题、sitemap
...
| `src/data/discovery-metadata.ts`                                          | 唯一的双语兴趣分类、编辑建议起看年龄及双语关键词                  |
...
| `src/data/raster-covers.ts`、`src/pages/covers/[slug].png.ts`             | 选定的精细实体封面由同一 SVG 几何生成 800×460 PNG，供目录按需加载 |
