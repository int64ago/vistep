...
每页具有自指向 canonical、中英文互相对应的 hreflang、x-default、语言、独立标题和描述、Open Graph / Twitter 信息，以及 1200 × 630 的 PNG 分享图。
...
每次构建自动更新路由、sitemap、语言替代链接、结构化数据和分享图。没有伪造每日更新时间、更新频率或优先级。Search Console 已提交稳定的 sitemap 地址，普通推送无需重复提交。

正式站允许收录；预览通过响应头设置 `noindex, nofollow`，并允许爬虫读取该头。不存在的路径实际返回 404 并设置 noindex。生产与预览使用不同产物目录和 Worker。
